...
{
	GetFeedListView()->ResizeColumns(event.GetSize().GetWidth());
	event.Skip();
}
...
void HtmlBoxEvents::OnHover(wxHtmlCellEvent & event)
{
	...
	{
		long selectedIndex = GetFeedListView()->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
		ItemInfo * info = (ItemInfo *) GetFeedListView()->GetItemData(selectedIndex);
		win->SetStatusText(wxString(info->link, wxConvUTF8), 0);
	}
}
